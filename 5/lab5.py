...
def enroll_to_subject(student, subject):
  ...
  for i in enrollment_list : 
      if i.get_student() == student and i.get_subject() == subject :       
...
def assign_grade(student, subject, grade):
  obj = search_enrollment_subject_student(subject, student) 
  if not isinstance(obj, Enrollment):
      return 'Not Found'
  if obj.get_grade()=='' :
      obj.set_grade(grade)
      return 'Done'
...
def get_student_record(student):
    ...
    for i in enrollment_list :
        if student == i.get_student() and i.get_grade() != '':
          subject = i.get_subject()
          dct[subject.get_id()] = [subject.get_name(),i.get_grade()]
    return dct
...
def get_student_GPS(student):
    ...
    for i in enrollment_list :
      if student == i.get_student() and i.get_grade() != '':
        sub_credit = i.get_subject().get_credit()
        all_credit += sub_credit
        cal_credit += grade_to_count(i.get_grade()) * sub_credit
    #print(cal_credit , all_credit)
    return cal_credit / all_credit
...
def register():
  ...
  enroll_to_subject(student_list[5], subject_list[2])  # 006 -> CS103x
