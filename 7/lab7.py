        ...
        for user in self.__user_list :
            for account in user.account_list :
                if isinstance(account, SavingAccount) \
                    and card_no == account.card.card_no :
                    return account
        return None
...
class Account:
    ...
    def __add__(self, amount):
        #transaction_type, amount, total, target_account=None
        self.__amount += amount
        self.add_transaction( Transaction('D', amount, self.__amount))
        return 'success'
    
    def __sub__(self, amount):
        if self.__amount >= amount :
            self.__amount -= amount
            self.add_transaction( Transaction('W', amount, self.__amount))
            return 'success'
        return 'error'
    
    def __rshift__(self, target_and_amount):
        target_account, amount = target_and_amount
        if isinstance(target_account, Account) and isinstance(amount, int) and self.__amount >= amount:
            self.__amount -= amount
            target_account.amount += amount
            self.add_transaction( Transaction('T', amount, self.__amount, target_account))
            target_account.add_transaction( Transaction('T-From', amount, self.__amount, self))
            return 'success'
        return 'error'
...
class Transaction:
    ...
    def __str__(self) :
        return f"Type:{self.transaction_type} - Amount:{self.amount} - Total:{self.total} - Target_account:{self.target_account}"
    #transaction_type, amount, total, target_account=None
...
class ATM_machine:
    ...
    def deposit(self, account, amount):
        if amount > 0 and isinstance(account, Account):
            if account + amount == 'success':
                self.__money += amount
                return 'success'
        #return 'error'

    def withdraw(self, account, amount):
        if self.__withdraw_limit >= amount > 0 and self.__money >= amount and isinstance(account, Account) :
            if account - amount == 'success' :
                self.__money -= amount
                return 'success'
        return 'error'
    ...
    def transfer(self, account, amount, target_account):
        if amount > 0 and isinstance(account, Account) and isinstance(target_account, Account):
            return account >> (target_account, amount)
...
class Seller:
    ...
    def paid(self, custumer_account, amount, seller_account):
        return custumer_account >> (seller_account, amount)
...
class EDC_machine:
    ...
    def paid(self, debit_card, amount, seller_account) :
        return debit_card.account >> (seller_account, amount)
