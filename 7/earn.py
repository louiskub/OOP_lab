...
class Bank:
    ...
    def search_account_from_card(self, card_no):
        for user in self.__user_list:
            for account in user.account_list:
                if isinstance(account, SavingAccount) and card_no == account.card.card_no:
                    return account
        return None
    ...
    def search_account_from_account_no(self, account_no):
        for user in self.__user_list:
            for account in user.account_list:
                if isinstance(account, Account) and account_no == account.account_no:
                    return account
                
    def search_seller(self, seller_name):
        for seller in self.__seller_list:
            if isinstance(seller, Seller) and seller_name == seller.name:
                return seller
...
class Account:
    ...
    def transfer(self, amount, target_account):
        return self >> (target_account, amount)
...
class ATM_machine:
    ...
    # (self, transaction_type, amount, total, target_account):
    def deposit(self, account, amount):
        if amount > 0 and isinstance(account, Account):
            if account.__add__(amount) == 'complete':
                self.__money += amount
                return 'success'
        return 'error'
    ...
    def withdraw(self, account, amount):
        if amount > 0 and isinstance(account, Account):
            if account.__sub__(amount) == 'complete':
                self.__money -= amount
                return 'success'
        return 'error'
...
class Seller:
    ...
    def search_edc_from_no(self, edc_no):
        for edc in self.__edc_list:
            if isinstance(edc, EDC_machine) and edc_no == edc.edc_no:
                return edc
            
    def paid(self, card, amount, seller_account):
        return card.account >> (seller_account, amount)
