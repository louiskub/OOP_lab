...
class Account :
  ...
  def deposit_balance(self, transaction) :
  ...
  def withdraw_balance(self, transaction) :
  ...
  def transfer_balance(self, transaction) :
    if isinstance(transaction, Transaction) and transaction.type == 'T' :
      if transaction.target_account == self :
        ...
        if total + transaction.amount <= self.max_deposit and self.balance >= transaction.amount :
          ...
          transaction.target_account.transfer_balance(target_transaction)
...
class Atm :
  ...
  def deposit(self, account, amount) :
    if amount > 0 and isinstance(account, Account):
      if account.deposit_balance( Transaction('D', amount, datetime.now(), self.atm_id)) == 'success':
  ...
  def withdraw(self, account, amount) :
    if self.total_cash >= amount > 0 and isinstance(account, Account) :
      if account.withdraw_balance( Transaction('W', amount, datetime.now(), self.atm_id)) == 'success' :
        self.total_cash -= amount
        return 'success'
    return 'error'
  ...
  def transfer(self, account, target_account, amount) :
    if amount > 0 and isinstance(account, Account) and isinstance(target_account, Account) :
      if account.transfer_balance( Transaction('T', amount, datetime.now(), self.atm_id, target_account)) == 'success' :
        return 'success'
    return 'error'
