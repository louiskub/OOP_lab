...
class Account:
    ...
    def check_max_per_day(self, transaction):
        ...
        for trans in self.__transaction_list:
            if trans.date_time.date() == target_date:
                if (trans.type == 'W' or (trans.type == 'T' and self != trans.target_account)) and self.__max_per_day >= trans.amounts:
                    withdraw_amounts += trans.amounts
        if self.__max_per_day - withdraw_amounts >= transaction.amounts:
            return self.__max_per_day - withdraw_amounts
        return self.__max_per_day                    
    
    def deposit(self, transaction):
        if isinstance(transaction, Transaction) and transaction.type == 'D':
    ...
    def withdraw(self, transaction):
        if isinstance(transaction, Transaction) and transaction.type == 'W':
            if self.__balance >= transaction.amounts <= self.check_max_per_day(transaction):
                self.__balance -= transaction.amounts
                transaction.total = self.__balance
                self.add_transaction(transaction)
                return "Success"
    ...
    def transfer(self, transaction):
        if isinstance(transaction, Transaction) and transaction.type == 'T':
...
class ATM:
    ...
    def deposit(self, account, amounts):
        if isinstance(account, Account) and amounts > 0:
            if account.deposit(Transaction(self.machine_id, 'D', amounts, datetime.now())) == "Success":
    ...
    def withdraw(self, account, amounts):
        if isinstance(account, Account) and self.balance >= amounts > 0:
            if account.withdraw(Transaction(self.machine_id, 'W', amounts, datetime.now())) == "Success":
                self.__balance -= amounts
                return "Success"
            return "Error"
        return "Error"
    ...
    def transfer(self, account, target_account, amounts):
        if isinstance(account, Account) and isinstance(target_account, Account) and amounts > 0:
            if account.transfer(Transaction(self.machine_id, 'T', amounts, datetime.now(), target_account)) == "Success":
                return target_account.transfer(Transaction(self.machine_id, 'T', amounts, datetime.now(), target_account))
            return "Error"
        return "Error"                                     
...
# print(atm2.deposit(atm2.insert_card(bank, harry_card, "1234"), 2000))
