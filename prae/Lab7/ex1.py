...
class Account:
    ...
    def __add__(self, amounts): # add money to account
        if type(amounts) == int or type(amounts) == float and amounts > 0:
            self.__balance += amounts
            return "Success"
        return "Errror"
    
    def __sub__(self, amounts): # take money out of account
    ...
    def check_condition(self, type, amounts, condition, dnt_account = None):
        if type == 'D' or type == 'W':
            if condition:
                self.add_transaction(Transaction(type, amounts, self.__balance))
                return "Success"
        elif type == 'T' or type == 'P':
            if condition:
                self.add_transaction(Transaction(type, amounts, self.__balance, dnt_account))
                dnt_account.add_transaction(Transaction(type, amounts, dnt_account.balance, dnt_account))
                return "Success"
        return "Error"    
    
    def deposit(self, amounts):
        return self.check_condition('D', amounts, self + amounts == "Success")
    
    def withdraw(self, amounts):
        return self.check_condition('W', amounts, self - amounts == "Success")
    
    def transfer(self, from_where, amounts, target_account):
        return self.check_condition('T', amounts, self.__max_transfer >= amounts and self >> [target_account, amounts], target_account)                       
...
class SavingsAccount(Account):
    ...
    def paid(self, amounts, seller_account):
        return self.check_condition('P', amounts, self >> [seller_account, amounts], seller_account)
