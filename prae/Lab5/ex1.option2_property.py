...
class Subject:
    ...
    sub_teacher = property(get_teacher, assign_teacher)
...
def get_teacher_teach(subject): # Teacher
    if isinstance(subject, Subject) and subject.sub_teacher != None:
        return subject.sub_teacher
    else:
        return "Not Found"
...
def get_student_record(student):
    ...
    for enrollment in enrollment_list:
        if enrollment.student == student:
            record[enrollment.subject.sub_id] = [enrollment.subject.sub_name, enrollment.grade]
    return record
...
def get_student_GPS(student):
    ...
    for sub_id, sub_grade in enrolled_sub.items():
        ...
        gps += subject.sub_credit * grade_to_count(sub_grade[1])
        credit_taken += subject.sub_credit
...
def list_student_enrolled_in_subject(subject_id):
    ...
    for enrollment in filter_student_list:
        student_dict[enrollment.student.stu_id] = enrollment.student.stu_name
    return student_dict
...
def list_subject_enrolled_by_student(student_id):
    ...
    for enrollment in filter_subject_list:
        subject_dict[enrollment.subject.sub_id] = enrollment.subject.sub_name
    return subject_dict                               
...
print(list_student_enrolled_in_subject(subject_list[0].sub_id))
print("")
...
print([i.student.stu_id for i in lst])
print("")
...
print(enroll.subject.sub_id, enroll.student.stu_id)
print("")
