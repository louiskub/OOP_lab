        ...
        if Member.check_email(email):
            if Member.check_phone_number(phone_number) and len(phone_number) == 10:
                if Member.check_password(password) and len(password) >= 8:
                    self.add_member(
                        Member(name, email, phone_number, birthday, password)
                    )
...
class Order:
    ...
    @property
    def total(self):
        self.__total = 0  # รีเซ็ตค่าเป็น 0 ก่อนที่จะคำนวณยอดรวมใหม่
    ...
    def apply_coupon(self, coupon):
        if self.__total >= coupon.min_purchase:
            self.__total -= coupon.get_discount_amount()  # ลดราคาตามค่าส่วนลดจากคูปอง
            return True
        else:
            return False
...
def create_cabana():
    ...
    activity_relax_zone = []
    activity_relax_zone.append(
        Cabana("P01", "S", "Activity and Relax")
    )  # Activity and Relax Zone
...
coupon_test = Coupon("CODE123", 100, 500, date(2022, 2, 2), date(2025, 2, 10))

coupon_enter = my_order.enter_coupon(coupon_test)
...
if my_order.apply_coupon(coupon_test):
    ...
    print(
        "Coupon cannot be applied. Total amount does not meet minimum purchase requirement."
    )

print(total)

james = Member("Louis", "james123@asd.com", "0812345678", date(2004, 4, 18))


Customer1 = dkub.add_member(james)

Booking_test = Booking(james, my_order)

Booking1 = james.add_booking(Booking_test)


print(james.get_booking_list())


@app.get("/order_detail", tags=["order_detail"])
def bookinglist():
    return james.get_booking_list()
